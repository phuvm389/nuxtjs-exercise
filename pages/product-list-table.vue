<template>
  <div class="product-list container mx-auto">
    ...
    <div v-if="items.length > 0" className="product-list__list m-5">
      <TableList :data="items" :columns="productColumns">
        <!-- header -->
        ...
        <template #image="{ value }">
          <div class="w-52">
            <img :src="`http://localhost:5000/${value.thumb}`" />
          </div>
        </template>
        ...
        <template #action="{ value }">
          ...
          <NuxtLink :to="`/product/edit/${value.productId}`">
            <font-awesome-icon icon="fa-pencil-alt" />
          </NuxtLink>

          <button class="link" @click="handleOpenModal(value.productId)">
